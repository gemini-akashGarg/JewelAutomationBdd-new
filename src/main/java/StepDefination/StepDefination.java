...
public class StepDefination {
    ...
    public void enter_password() {
        ...
        DriverAction.waitSec(2);
        try {
            DriverAction.click(By.xpath("//button[text()='Login']"), "Login Button");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void verify_logout_button_is_visible_or_not() {
        try {
            ...
            DriverAction.click(By.xpath(" //div[@class='d-flex']"));
    ...
    public void click_on_the_autolytics_screen() {
        ...
        if (present == true) {
            try {
                DriverAction.click(By.xpath("(//div[text()='Autolytics'])[2]"), "Autolytics Button");
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void verify_the_bridge_token_button_is_clickable_or_not() {
        ...
        if (present == true) {
            ...
            try {
                DriverAction.click(By.xpath("(//div[text()='Bridge Token'])[2]"), "Bridge token Button");
                String s = DriverAction.getCurrentURL();

                if (s.contains("bridge-token")) {
                    GemTestReporter.addTestStep("Bridge Token window", "Window is opened successfully", STATUS.PASS);
                } else {
                    GemTestReporter.addTestStep("Bridge Token window", "Window is not opened successfully", STATUS.FAIL);
    ...
    public void click_on_copy_bridge_token() {
        ...
        try {
            ...
            if (present == true) {
                try {
                    DriverAction.click(By.xpath("//div[@class='copyIcon col-2 d-flex justify-content-center align-items-center']"), "copy button");
                    DriverAction.waitSec(2);
    ...
    public void verify_the_alert_when_copy_button_is_clicked() {
        ...
        if (present == true) {
            try {
                ...
                if (se.equals("Copied !")) {
                    GemTestReporter.addTestStep("Alert status", "Expected alert:Copied successfully", STATUS.PASS);
                } else {
                    GemTestReporter.addTestStep("Alert status", "Expected alert:Copied successfully", STATUS.FAIL);
                }
            } catch (Exception e) {
                e.printStackTrace();
                GemTestReporter.addTestStep("Status of Bridge Token button", "Clicked Failed", STATUS.FAIL);
    ...
    public void verify_the_alert_when_change_token_button_is_clicked() {
        ...
        try {
            ...
            try {
                DriverAction.click(By.xpath("//div[text()='Change Token']"), "Change token button");
                DriverAction.waitSec(1);
            ...
            if (fi.contains("Bridge Token Changed Successfully.")) {
                GemTestReporter.addTestStep("Alert status", "Expected alrt:Changed successfully", STATUS.PASS, DriverAction.takeSnapShot());
            } else {
                GemTestReporter.addTestStep("Alert status", "Expected alrt:Changed successfully", STATUS.FAIL, DriverAction.takeSnapShot());
            }
            DriverAction.waitSec(2);
            String time = DriverAction.getElement(By.xpath("//div[@class='createdOn mt-3']")).getText();
            System.out.println(time);
            String timep = time.substring(10, 20);
            String date = timep;
            ...
            String times = time.substring(21, 29);
    ...
    public void click_and_verify_the_logout_button() {
        ...
        DriverAction.click(By.xpath(" //div[text()='Home']"));
        try {
            DriverAction.click(By.xpath("//div[@class='d-flex']"), "Profile Element");
            DriverAction.waitSec(3);
            DriverAction.click(By.xpath("//div[@class='dropdown-item mx-auto text-center']"), "Logout button");
            DriverAction.waitSec(2);
            String title = DriverAction.getTitle("");
    ...
    public void verify_the_cards_present_on_the_home_screen() {
        DriverAction.waitSec(2);
        DriverAction.click(By.xpath(" //div[text()='Home']"));
        GemTestReporter.addTestStep("Successfully chlicked", "Jewel Window", STATUS.INFO, DriverAction.takeSnapShot());

        try {
            List num = DriverAction.getElements(By.xpath("//div[@class='card-body ms-2']"));
            if (num.size() == 2) {
                GemTestReporter.addTestStep("Numner of Cards Present ", "Expcted :2", STATUS.PASS);
            } else {
                GemTestReporter.addTestStep("Numner of Cards Present ", "Expcted :2", STATUS.FAIL);
            }
...
//            GemTestReporter.addTestStep("Login Success", "Successfully Logged in!!", STATUS.PASS);
            ...
            DriverAction.waitSec(5);
            try {
                DriverAction.click(By.xpath("//img[@class='collapseBtn ms-3 mt-3 mb-2']"), "Three Lines button");
            ...
            try {
                DriverAction.click(By.xpath(" //div[@class='d-flex']"), "Usernane button");
                GemTestReporter.addTestStep("CLick on the username", "Successfully clicked", STATUS.PASS);
                DriverAction.waitSec(2);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void click_on_the_sign_up_button() {
        DriverAction.waitSec(3);
        DriverAction.click(By.xpath("//span[text()='Sign up']"));
        DriverAction.waitSec(3);
